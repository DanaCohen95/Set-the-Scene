...
class StableDiffusion(nn.Module):
    def __init__(self, device, model_name='stabilityai/stable-diffusion-2-base', concept_name=None, latent_mode=True):
        super().__init__()
    ...
    def train_step(self, text_embeddings, inputs, train_step, total_steps, guidance_scale=100):
        ...
        # gaussian_mean = 0.7 - train_step / total_steps * 0.6
        # t = torch.normal(gaussian_mean, 0.2, [1], device=self.device)
        # t = (t.clamp(0.02, 0.98) * self.num_train_timesteps).to(torch.long)
        # logger.info(f'{gaussian_mean}, {t}')
